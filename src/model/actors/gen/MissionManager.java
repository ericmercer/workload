...
public MissionManager(ComChannelList inputs, ComChannelList outputs) {
	...
	initializeRX_PS(inputs, outputs, RX_VO, RX_OP, RX_PS, IDLE);
	initializeRX_VO(inputs, outputs, RX_PS, RX_OP, RX_VO, IDLE);
	initializeIDLE(inputs, outputs, RX_PS, RX_VO, IDLE, RX_OP);
	initializeRX_OP(inputs, outputs, RX_PS, RX_VO, RX_OP, IDLE);
	startState(IDLE);
}
 public void initializeRX_PS(ComChannelList inputs, ComChannelList outputs, State RX_VO, State RX_OP, State RX_PS, State IDLE) {
	// (RX_PS,[A=PS_NEW_SEARCH_AOI_MM],[],0,NEXT,1.0)x(IDLE,[D=MM_STOP_LISTEN_TO_PS_MM],[AREA_OF_INTEREST=NEW])
	RX_PS.add(new Transition(_internal_vars, inputs, outputs, IDLE, Duration.NEXT.getRange(), 0, 1.0) {
		@Override
	...
	// (RX_PS,[A=PS_NEW_SEARCH_AOI_MM,A=OP_POKE_MM],[],1,NEXT,1.0)x(RX_OP,[A=MM_ACK_OP,D=MM_STOP_LISTEN_TO_PS_MM],[AREA_OF_INTEREST=NEW])
	RX_PS.add(new Transition(_internal_vars, inputs, outputs, RX_OP, Duration.NEXT.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
			if(!ParentSearch.AUDIO_PS_MM_COMM.PS_NEW_SEARCH_AOI_MM.equals(_inputs.get(Channels.AUDIO_PS_MM_COMM.name()).getValue())) {
				return false;
	...
	// (RX_PS,[A=PS_NEW_SEARCH_AOI_MM,A=VO_POKE_MM],[],1,NEXT,1.0)x(RX_VO,[A=MM_ACK_VO,D=MM_STOP_LISTEN_TO_PS_MM],[AREA_OF_INTEREST=NEW])
	RX_PS.add(new Transition(_internal_vars, inputs, outputs, RX_VO, Duration.NEXT.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
			if(!ParentSearch.AUDIO_PS_MM_COMM.PS_NEW_SEARCH_AOI_MM.equals(_inputs.get(Channels.AUDIO_PS_MM_COMM.name()).getValue())) {
				return false;
 ...
 public void initializeRX_VO(ComChannelList inputs, ComChannelList outputs, State RX_PS, State RX_OP, State RX_VO, State IDLE) {
	// (RX_VO,[A=VO_TARGET_SIGHTED_F_MM],[],0,NEXT,1.0)x(IDLE,[D=MM_STOP_LISTEN_TO_VO_MM],[TARGET_SIGHTED_F=TRUE])
	RX_VO.add(new Transition(_internal_vars, inputs, outputs, IDLE, Duration.NEXT.getRange(), 0, 1.0) {
		@Override
		public boolean isEnabled() { 
	...
	// (RX_VO,[A=VO_TARGET_SIGHTED_F_MM,A=OP_POKE_MM],[],1,NEXT,1.0)x(RX_OP,[A=MM_ACK_OP,D=MM_STOP_LISTEN_TO_VO_MM],[TARGET_SIGHTED_F=TRUE])
	RX_VO.add(new Transition(_internal_vars, inputs, outputs, RX_OP, Duration.NEXT.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
			if(!VideoOperator.AUDIO_VO_MM_COMM.VO_TARGET_SIGHTED_F_MM.equals(_inputs.get(Channels.AUDIO_VO_MM_COMM.name()).getValue())) {
				return false;
	...
	// (RX_VO,[A=VO_TARGET_SIGHTED_F_MM,A=PS_POKE_MM],[],1,NEXT,1.0)x(RX_PS,[A=MM_ACK_PS,D=MM_STOP_LISTEN_TO_VO_MM],[TARGET_SIGHTED_F=TRUE])
	RX_VO.add(new Transition(_internal_vars, inputs, outputs, RX_PS, Duration.NEXT.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
			if(!VideoOperator.AUDIO_VO_MM_COMM.VO_TARGET_SIGHTED_F_MM.equals(_inputs.get(Channels.AUDIO_VO_MM_COMM.name()).getValue())) {
				return false;
 ...
 public void initializeRX_OP(ComChannelList inputs, ComChannelList outputs, State RX_PS, State RX_VO, State RX_OP, State IDLE) {
	// (RX_OP,[A=OP_SEARCH_COMPLETE_MM],[],0,NEXT,1.0)x(IDLE,[D=MM_STOP_LISTEN_TO_OP_MM],[SEARCH_COMPLETE=TRUE])
	RX_OP.add(new Transition(_internal_vars, inputs, outputs, IDLE, Duration.NEXT.getRange(), 0, 1.0) {
		@Override
		public boolean isEnabled() { 
	...
	// (RX_OP,[A=OP_SEARCH_COMPLETE_MM,A=VO_POKE_MM],[],1,NEXT,1.0)x(RX_VO,[A=MM_ACK_VO,D=MM_STOP_LISTEN_TO_OP_MM],[SEARCH_COMPLETE=TRUE])
	RX_OP.add(new Transition(_internal_vars, inputs, outputs, RX_VO, Duration.NEXT.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
			if(!Operator.AUDIO_OP_MM_COMM.OP_SEARCH_COMPLETE_MM.equals(_inputs.get(Channels.AUDIO_OP_MM_COMM.name()).getValue())) {
				return false;
	...
	// (RX_OP,[A=OP_SEARCH_COMPLETE_MM,A=PS_POKE_MM],[],1,NEXT,1.0)x(RX_PS,[A=MM_ACK_PS,D=MM_STOP_LISTEN_TO_OP_MM],[SEARCH_COMPLETE=TRUE])
	RX_OP.add(new Transition(_internal_vars, inputs, outputs, RX_PS, Duration.NEXT.getRange(), 1, 1.0) {
		@Override
		public boolean isEnabled() { 
			if(!Operator.AUDIO_OP_MM_COMM.OP_SEARCH_COMPLETE_MM.equals(_inputs.get(Channels.AUDIO_OP_MM_COMM.name()).getValue())) {
				return false;
