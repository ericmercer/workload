...
//				System.out.print("");
	...
	private void getEnabledTransitions()
	{
		...
		for(Map.Entry<IActor, ITransition> transitionEntry : transitions.entrySet() ) {
			...
			boolean added = _clock.addTransition(actor, transition, duration);
	...
	public int getDuration(Range range)
	{
		...
		switch(_duration) {
			...
			case MEAN:
				return mean;
			case MIN_MAX:
				return Verify.getIntFromList(new int[]{min, max});
			case MIN_MAX_MEAN:
				return Verify.getIntFromList(new int[]{min, max,(min+max)/2});
			default:
				return 1;
		}
	}
